...
function startAnimation() {
    ...
    animationInterval = setInterval(() => {
        ...
        roboImage.src = `../assets/images/robo/robo${imageIndex + 1}.png`; // Corrigido o caminho para a animação do robô
    }, 70); 
}

// Para a animação do robô e reseta a imagem
function stopAnimation() {
    clearInterval(animationInterval); 
    const roboImage = document.getElementById('robo-image');
    roboImage.src = `../assets/images/robo/robo.png`; // Corrigido o caminho para a imagem do robô
}
...
if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
    ...
    recognition.onresult = (event) => { 
        const speechResult = event.results[0][0].transcript; questionInput.value = speechResult; 
        getAnswer(); 
    };
